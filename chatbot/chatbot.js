...
let isReady = false;
const botIconHTML = '<img src="chatbot/chatbot.png" alt="Chatbot Icon" class="bot-icon">';
...
worker.onmessage = function (e) {
    if (e.data.status === 'modelLoaded') {
        ...
        isReady = true;
        chatbox.innerHTML += `<div class="bot">${botIconHTML}<p class="bot-message"><em>Hello! How can I help you today?</em></p></div>`;
        displayInitialFollowUpQuestions();
        hideLoadingIndicator();
        enableUserInput();
    } else if (e.data.status === 'error') {
        chatbox.innerHTML += `<div class="bot">${botIconHTML}<p class="bot-message"><em>Oops! Something went wrong. Please try again later.</em></p></div>`;
        hideLoadingIndicator();
    }
};
...
async function loadModel() {
    try {
        ...
        chatbox.innerHTML += `<div class="bot">${botIconHTML}<p class="bot-message"><em>Oops! Something went wrong while loading the chatbot. Please try again later.</em></p></div>`;
        hideLoadingIndicator();
    }
}
...
async function handleUserInput(message) {
    try {
        ...
        const loadingIndicatorHTML = `<div class="bot">${botIconHTML}<p class="bot-message"><em><span class="typing-indicator"><span class="dot"></span><span class="dot"></span><span class="dot"></span></span></em></p></div>`;
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            ...
            for (let j = 0; j < words.length; j++) {
                currentHTML += words[j] + ' ';
                element.innerHTML = `<em></em> ${currentHTML}`;
                chatbox.scrollTop = chatbox.scrollHeight;
