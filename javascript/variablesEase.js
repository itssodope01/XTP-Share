...
function loadCloudAccounts() {
    ...
    if (userCloudAuthTypes.length === 0) {
...
function updateUI(selectedFilesContainer) {
    ...
    if (numFiles > 0) {
        ...
        Border.style.border = '4px solid var(--file-drop-box-border)';
        [attachmentContainer]
...
function updateButtonPositions() {
    ...
    if (windowWidth >700 && windowWidth < 950 ) {
        largeScreenPosition = '89.4%';
    }else if (windowWidth < 700) {
        largeScreenPosition = '88.4%';
    } else {
        largeScreenPosition = '84.4%';
    }

    const numFiles = selectedFilesContainer.children.length;
    let up = windowWidth < 700 ? 2 : 6;

    clearAllButton.style.left = numFiles > 1 ? largeScreenPosition : '50%';
    AddFilesButton.style.left = numFiles > up ? largeScreenPosition : '75%';

    if (clearAllButton.style.left === largeScreenPosition) {
        clearAllButton.disabled = false;
        clearAllButton.style.visibility = 'visible';
    }

    if (AddFilesButton.style.left === largeScreenPosition) {
        AddFilesButton.disabled = false;
        AddFilesButton.style.visibility = 'visible';
    }

    uploadButton.style.display = numFiles > up ? 'none' : 'block';
}
...
async function updateAttachmentContainerBorderColor() {
    if(document.querySelector('select[name="to"]').value) {
